...
PATCHES=(
	...
	"${FILESDIR}"/${P}-query-surface-attributes.patch
	"${FILESDIR}"/${P}-create-surfaces-v2.patch
	"${FILESDIR}"/${P}-ConstrainedBaseline.patch
	"${FILESDIR}"/${P}-fallback-x.patch
	"${FILESDIR}"/${P}-debug-object-heap.patch
	"${FILESDIR}"/${P}-null-check.patch
	"${FILESDIR}"/${P}-use-vaDestroyBuffer.patch
	"${FILESDIR}"/${P}-vp9-support.patch
)
